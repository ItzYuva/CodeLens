...
CodeLens lets you paste a GitHub repo URL and ask natural language questions about the codebase. It uses **Vectorless RAG** -- a tree-based retrieval approach that parses code into a hierarchy, summarizes each node, and traverses the tree intelligently at query time.

> Unlike traditional RAG that chunks code into flat pieces and relies on cosine similarity, CodeLens understands your codebase *structurally* -- it knows which files contain which classes, which functions call what, and navigates to the right code the way a developer would.
